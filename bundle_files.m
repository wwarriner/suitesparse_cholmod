...
[ this_dir, this_name ] = fileparts( mfilename( 'fullpath' ) );

%% ARGS
if nargin < 1
    target = fullfile( this_dir, DEFAULT_OUT_NAME );
end

%% GET MEX FILE LIST
mex_files = dir( fullfile( this_dir, '**', '*.mex*' ) );

%% GET M FILE LIST
m_files = dir( fullfile( this_dir, '**', '*.m' ) );
...
pdf_files = dir( fullfile( this_dir, '**', 'Doc', '*.pdf' ) );
sub_licenses = dir( fullfile( this_dir, '**', 'Doc', '*.txt' ) );
...
other_docs = dir( fullfile( this_dir, '*.txt' ) );
other_docs = [ other_docs; dir( fullfile( this_dir, '*.md' ) ) ];
